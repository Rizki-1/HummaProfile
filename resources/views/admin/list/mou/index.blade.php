                ...
                <div class="col-md-4 mb-4 flex-row">
                    <div class="card">
                        <div class="image-container">
                            <img src="{{ asset('storage/mou/' . $row->foto_mou) }}" class="image-content"
                                alt="Thumbnail {{ $row->nama_mou }}">
                        </div>
                        <div class="image-hover">
                            <div class="image-detail">
                                ...
                                <div class="action-container">
                                    <a href="{{ route('mou.edit', $row->id) }}"><i class="link-icon edit-icon"
                                            data-feather="edit"></i></a>
                                    <form nameMoU="{{ $row->nama_mou }}" action="{{ route('mou.destroy', $row->id) }}"
                                        method="POST" class="berita hapus">
                                        @csrf
                                        @method('DELETE')
                                        <button type="submit" class="button-delete"><i class="link-icon trash-icon"
            ...
            @empty
                @if (!request('query'))
                    <p class="fw-bold text-center">Tidak ada MoU yang terdaftar. <a
                            href="{{ route('mou.create') }}">Daftar!</a>
                    </p>
                @else
